...
do
    ...
    do
        ...
        if [ "$split" = "test" ] ;
        then
            output_file="rank_eval_no_in_out_links_"${folder}"_"${trial%?}"_"${checkpoint%???}"_test.txt"
        else
            output_file="rank_eval_no_in_out_links_"${folder}"_"${trial%?}"_"${checkpoint%???}".txt"
        fi
        ...
        then
            kge $split . --eval.type ranking_evaluation --checkpoint $checkpoint \
                        --ranking_evaluation.query_types.sp_ 1.0 \
                        --ranking_evaluation.query_types._po 1.0 \
                        --ranking_evaluation.query_types.s_o 1.0 \
                        ...
                        --ranking_evaluation.query_types.s_^ 0.0 \
                        --ranking_evaluation.query_types.^_o 0.0 \
    ...
    if [ "$split" = "test" ] ;
    then
        python ../../create_csv_summary.py --folder $folder --trial $trial --eval rank_eval_no_in_out_links --test_data True --test_data_n_times False
    else
        python ../../create_csv_summary.py --folder $folder --trial $trial --eval rank_eval_no_in_out_links --test_data False --test_data_n_times False
    fi
    cd ..
done
cd ..
