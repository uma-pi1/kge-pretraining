...
do
    ...
    do
        ...
        if [ "$split" = "test" ] ;
        then
            output_file="rank_eval_"${folder}"_"${trial%?}"_"${checkpoint%???}"_test.txt"
        else
            output_file="rank_eval_"${folder}"_"${trial%?}"_"${checkpoint%???}".txt"
        fi
    ...
    if [ "$split" = "test" ] ;
    then
        python ../../create_csv_summary.py --folder $folder --trial $trial --eval rank_eval --test_data True --test_data_n_times False
    else
        python ../../create_csv_summary.py --folder $folder --trial $trial --eval rank_eval --test_data False --test_data_n_times False
    fi
    cd ..
done
cd ..
