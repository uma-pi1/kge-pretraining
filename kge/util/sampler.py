...
class BatchNegativeSample(Configurable):
    ...
    def score(self, model, indexes=None, query_type=None) -> torch.Tensor:
        ...
        if query_type is not None:
            ...
            if placeholder_slot != -1:
                ...
                triples[:, placeholder_slot] = torch.tensor(
                    model.pseudo_indices[placeholder_slot]
                ).expand(triples[:, placeholder_slot].size(0)).to(device)
        self.prepare_time += time.time()
