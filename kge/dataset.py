...
class Dataset(Configurable):
    ...
    def __init__(self, config, folder=None):
        ...
        #: downstream split name to (n,3) int32 tensor
        self._downstream_split: Dict[str, Tensor] = {}
    ...
    # TODO clean this up!
    @staticmethod
    def _load_downstream_split(
            filename: str, delimiter="\t", use_pickle=False
    ) -> Tensor:
        # if use_pickle:
        #     # check if there is a pickled, up-to-date version of the file
        #     pickle_suffix = Dataset._to_valid_filename(f"-{delimiter}.pckl")
        ...
        rows = np.loadtxt(filename, delimiter=delimiter)
        rows = torch.from_numpy(rows)
        return rows

    # TODO clean this up!
    def load_downstream_split(
            self,
            key: str,
            downstream_folder=None
    ) -> Tensor:
        """
        Load and return the downstream task split with the specified key.
        A downstream task split is an n x m tensor of form (ent_id, [values])
        where values are either binary indicators for m classes or a single
        regression value.
        """

        if key not in self._downstream_split:
            # self.ensure_available(key)
            # filename = self.config.get(f"dataset.files.{key}.filename")
            # filetype = self.config.get(f"dataset.files.{key}.type")
            # if filetype != "triples":
            ...
            # TODO these filenames should be created when dataset.yaml is created
            #   Then the commented snippet above should work, with modifications
            filename = "{}.del".format(key)
            # check that folder exists
            if not os.path.isdir(os.path.join(self.folder, downstream_folder)):
                raise ValueError("Folder {} not found for dataset {}.".format(
                    downstream_folder, self.config.get("dataset.name"))
                )
            rows = Dataset._load_downstream_split(
                os.path.join(self.folder, downstream_folder, filename)
            )
            self.config.log(
                f"Loaded {len(rows)} tuples from {key} downstream split"
            )
            self._downstream_split[key] = rows

        return self._downstream_split[key]
...
"""
    ...
    def downstream_split(self, split: str, folder=None) -> Tensor:
        """
        Return the downstream task split of the specified name.
        """
        return self.load_downstream_split(split, folder)
